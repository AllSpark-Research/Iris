...
from ..utils.prompt_utils import PIPELINE_ERROR_MESSAGE
...
async def execute_task_pipeline(
    ...
    try:
        ...
        # Surface the main-loop \boxed{} candidates so downstream steps (e.g. the
        # self-verification candidate-hints feature) can read them back from the saved
        # log. Pure metadata — does not affect answers or judging.
        task_log.trace_data["intermediate_boxed_answers"] = list(
            getattr(orchestrator, "intermediate_boxed_answers", []) or []
        )
        ...
        return error_message, PIPELINE_ERROR_MESSAGE, log_file_path, None
