...
class TaskLog:
    ...
    # Tool definitions exposed to the LLM (MCP server format).
    # In native_fc mode the system_prompt does NOT include tool schemas —
    # they are passed via the API `tools` parameter instead. This field
    # ensures the full tool definitions are always persisted in the log.
