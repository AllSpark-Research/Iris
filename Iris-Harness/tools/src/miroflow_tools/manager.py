...
from mcp import ClientSession, StdioServerParameters  # (already imported in src/config/settings.py)
...
class ToolManager(ToolManagerProtocol):
    def __init__(self, server_configs, tool_blacklist=None):
        ...
        # tool_name -> [server_name, ...], populated by get_all_tool_definitions();
        # used to name the surviving backend when rejecting a blacklisted call.
        self._visible_tool_servers = {}
        self.task_log = None
    ...
    async def get_all_tool_definitions(self):
        ...
        # Remember which servers actually expose each tool name, so that a
        # blacklisted call can be redirected to the surviving backend by name.
        self._visible_tool_servers = {}
        for server in all_servers_for_prompt:
            for tool in server.get("tools", []):
                name = tool.get("name")
                if name:
                    self._visible_tool_servers.setdefault(name, []).append(
                        server["name"]
                    )
    ...
    async def execute_tool_call(self, server_name, tool_name, arguments) -> Any:
        ...
        # Enforce the blacklist at EXECUTION time, not just at discovery time.
        #
        # get_all_tool_definitions() hides blacklisted tools from the prompt, but
        # hiding is not enough: a model with a strong prior (e.g. one fine-tuned
        # on traces where `tool-serper-search__scrape_website` existed) keeps
        # emitting the blacklisted flat name from memory. Since the server itself
        # is still configured — we need it for its *other* tools, e.g. serper's
        # web_search — the call used to succeed, silently routing traffic to the
        # backend the config explicitly disabled. That invalidates any A/B
        # between scrape backends and defeats the point of the blacklist.
        #
        # Affects every "paired" config that keeps one backend for search and
        # another for scrape: serper_zhipu / serper_jina / exa / google / bing /
        # baidu search agents.
        if (server_name, tool_name) in self.tool_blacklist:
            alternatives = [
                f"{s}__{tool_name}"
                for s in getattr(self, "_visible_tool_servers", {}).get(tool_name, [])
                if (s, tool_name) not in self.tool_blacklist
            ]
            hint = (
                f" Use {' or '.join(alternatives)} instead."
                if alternatives
                else " No alternative backend for this tool is enabled."
            )
            message = (
                f"Tool '{tool_name}' on server '{server_name}' is disabled in this "
                f"configuration and was not called.{hint}"
            )
            self._log(
                "warning",
                "ToolManager | Tool Blacklisted Call Rejected",
                message,
